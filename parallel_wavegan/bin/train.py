    ...
    def _train_step(self, batch):
        ...
        if self.steps > self.config["discriminator_train_start_steps"]:
            ...
            if not isinstance(p_, list):
                ...
                for (_, score_fake), (_, score_real) in zip(p_, p):
                    rls_loss_g += torch.mean((score_fake - score_real -1)**2)
                rls_loss_g = rls_loss_g / len(p_)

                self.total_train_loss["train/adversarial_loss"] += adv_loss.item()
                self.total_train_loss["train/adversarial_loss"] += rls_loss_g.item()
            ...
            gen_loss += 4.0 * adv_loss
            gen_loss += 0.4 * rls_loss_g
        ...
        if self.steps > self.config["discriminator_train_start_steps"]:
            ...
            if not isinstance(p, list):
                ...
                for (_, score_fake), (_, score_real) in zip(p_, p):
                    rls_loss_d += torch.mean((score_real - score_fake - 1)**2)
                rls_loss_d = rls_loss_d / len(p_)


                dis_loss = real_loss + fake_loss + (0.4 * rls_loss_d)
